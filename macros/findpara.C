...
public :
  ...
  Double_t        StepdS_rec_m[MaxHitInATrack];   //[HitNum_m]
  Double_t        StepdPhi_rec_m[MaxHitInATrack]; //[HitNum_m]
  Double_t        StepFi_rec_m[MaxHitInATrack];   //[HitNum_m]
  Double_t        StepdFi_rec_m[MaxHitInATrack];  //[HitNum_m]
  Double_t        StepSpace_rec_m[MaxHitInATrack];//[HitNum_m]
  Double_t        StepAngle_rec_m[MaxHitInATrack];//[HitNum_m]
...
void NtReader::Init()
{
  ...
  fout = new TFile("cf.root","recreate");
  tout = new TTree("t","study chain finder paramerters");
  
  tout->Branch("Npt",&HitNum_m,"Npt/I");
  tout->Branch("StepID",StepID_m,"StepID[Npt]/I");
  tout->Branch("StepTDC",StepTDC_m,"StepTDC[Npt]/I");
  tout->Branch("StepADC",StepADC_m,"StepADC[Npt]/I");
  tout->Branch("StepX",StepX_rec_m,"StepX[Npt]/D");
  tout->Branch("StepY",StepY_rec_m,"StepY[Npt]/D");
  tout->Branch("StepZ",StepZ_rec_m,"StepZ[Npt]/D");
  tout->Branch("StepS",StepS_rec_m,"StepS[Npt]/D");
  tout->Branch("StepPhi",StepPhi_rec_m,"StepPhi[Npt]/D");
  tout->Branch("StepdS",StepdS_rec_m,"StepdS[Npt]/D");
  tout->Branch("StepdPhi",StepdPhi_rec_m,"StepdPhi[Npt]/D");
  tout->Branch("StepFi",StepFi_rec_m,"StepFi[Npt]/D");
  tout->Branch("StepdFi",StepdFi_rec_m,"StepdFi[Npt]/D");
  tout->Branch("StepSpace",StepSpace_rec_m,"StepSpace[Npt]/D");
  tout->Branch("StepAngle",StepAngle_rec_m,"StepAngle[Npt]/D");
  tout->Branch("R",&R_sim,"R/D");
  tout->Branch("A",&A_sim,"A/D");
  tout->Branch("B",&B_sim,"B/D");
  tout->Branch("DCA",&DCA_sim,"DCA/D");
  tout->Branch("MeanBz",&MeanBz,"MeanBz/D");
  
  tout->Branch("X",&X0_rec_p,"X/D");
  tout->Branch("Y",&Y0_rec_p,"Y/D");
  tout->Branch("Z",&Z0_rec_p,"Z/D");
  tout->Branch("P",&P0_rec_p,"P/D");
  tout->Branch("Theta",&Theta0_rec_p,"Theta/D");
  tout->Branch("Phi",&Phi0_rec_p,"Phi/D");
  
  tout->Branch("P0",&P0_p,"P0/D");
  tout->Branch("Theta0",&Theta0_p,"Theta0/D");
  tout->Branch("Phi0",&Phi0_p,"Phi0/D");
  tout->Branch("X0",&X0,"X0/D");
  tout->Branch("Y0",&Y0,"Y0/D");
...
void NtReader::Loop()
{
  TVector3 pV3,pV3_seed,pV3_diff,pV3_diff_pre;
  if (fChain == 0) return;
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(HitNum_m<5 || Smax<50.) continue;
    
    
    for(int j=0;j<HitNum_m;j++) {
      StepFi_rec_m[j] = atan2(StepY_rec_m[j]-B_sim, StepX_rec_m[j]-A_sim);
    }
    
    StepdS_rec_m[0] = 0.0;
    StepdPhi_rec_m[0] = 0.0;
    StepdFi_rec_m[0] = 0.0;
    StepSpace_rec_m[0] = 0.0;
    StepAngle_rec_m[0] = 0.0;
    
    for(int i=1;i<HitNum_m;i++) {
      pV3_seed.SetXYZ(StepX_rec_m[i-1],StepY_rec_m[i-1],StepZ_rec_m[i-1]);
      pV3.SetXYZ(StepX_rec_m[i],StepY_rec_m[i],StepZ_rec_m[i]);
      pV3_diff = pV3 - pV3_seed;
      if(i==1) pV3_diff_pre = pV3_diff;
      
      StepSpace_rec_m[i] = pV3_diff.Mag();
      StepAngle_rec_m[i] = pV3_diff.Angle(pV3_diff_pre);
      StepdS_rec_m[i] = StepS_rec_m[i] - StepS_rec_m[i-1];
      StepdPhi_rec_m[i] = StepPhi_rec_m[i] - StepPhi_rec_m[i-1];
      StepdFi_rec_m[i] = StepFi_rec_m[i] - StepFi_rec_m[i-1];
      pV3_diff_pre = pV3_diff;
    }
    tout->Fill();
    
    if (!((jentry+1)%1000)) {
      cout<<jentry+1<<"/"<<nentries<<" events processed ...... \r";
...
void findpara(const char* file="infile.root")
{
    NtReader r(file);
    r.Loop();
}
